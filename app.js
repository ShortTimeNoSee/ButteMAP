...
function satisfyUnitsSection(section, sectionItems, poolsByList, takenSet, usedSet) {
    ...
    for (const listName of allow) {
        ...
        for (const c of chosen)
            if (c.from === listName) have += (c.units || 0);
        while (have < need) {
...
function evalCrossListUnits(program, takenSet, usedSet) {
    ...
    for (const s of sections) {
        ...
        for (const item of (s.items || [])) {
            for (const opt of (item.any_of || [])) {
                arr.push({
                    code: String(opt.code).trim().toUpperCase(),
                    units: opt.units || 0,
                    itemRef: item
                });
            }
    ...
    for (const name of Object.keys(sectionPool)) {
        for (const h of hitsBySection[name]) {
            if (!usedCodes.has(h.code) && !usedSet.has(h.code)) leftovers.push({
                ...h,
                name
            });
        }
...
function evaluateProgram(detail, transcript, countPlanned = false, skipGESections = false) {
    ...
    const sections = (detail.sections || []).map(s0 => {
        const sec = coerceSelectCountRule({
            ...s0
        });
        return {
            name: sec.name,
    ...
    for (const s of sections.filter(x => isAllish({
            detail
        }, x))) {
    ...
    for (const s of sections.filter(
            x => ruleTypeOf(x.rule.type) === 'COUNT' && !sectionIsInCrossList({
                detail
            }, x.name)
        )) {
        if (skipGESections && isGESection(s.name)) continue;
    ...
    for (const r of crossRules) {
        ...
        crossFindings.push({
            ...
            need_total: minTotal,
            per_list_min_status: perListShort.map(([lname, min]) => ({
                list: lname,
                have: perListUnits[lname] || 0,
                need: min
            }))
        });
    }
            ...
            for (const item of s.items) {
                if (item.matched) {
                    secRow.items.push({
                        type: 'single',
                        met: true,
                        matched: item.chosen.code,
                        options: [item.chosen.code]
                    });
                } else {
                    ...
                    secRow.items.push({
                        type: finalOptions.length > 1 ? 'or' : 'single',
                        met: false,
                        matched: null,
...
function evalPrereqNode(node, takenSet, hsBackground) {
    ...
    if (node.text) {
        ...
        if (txt.includes('high school chemistry') || (txt.includes('year of') && txt.includes('chemistry'))) {
            ...
            return {
                met,
                options: met ? [] : ['HS Chemistry (or take CHEM 11/CHEM 51)'],
                singleOption: 'HS_CHEMISTRY'
            };
        }
        ...
        if (txt.includes('intermediate algebra') || (txt.includes('algebra') && txt.includes('equivalent'))) {
            ...
            return {
                met,
                options: met ? [] : ['Intermediate Algebra'],
                singleOption: 'HS_INT_ALGEBRA'
            };
        }
        ...
        if (op === 'OR') {
            const paths = opts.map(opt => {
                if (typeof opt === 'string') {
                    ...
                    return {
                        courses: [opt, ...subResult.courses],
                        options: subResult.options
                    };
                }
                return {
                    courses: [],
    ...
    const flattenOptions = (opts, op) => {
        ...
        for (const opt of opts) {
            if (typeof opt === 'string') {
                ...
                items.push({
                    op: opt.op,
                    items: flattenOptions(opt.options, opt.op)
                });
            }
        }
    ...
    for (const f of yearFiles) {
        try {
            ...
            if (Array.isArray(pj.programs)) {
                ...
                state.programs.push(...pj.programs.map(p => ({
                    ...p,
                    source_file: f,
                    year_label
                })));
...
function renderTerms() {
    ...
    state.terms.forEach((term, tIdx) => {
        ...
        $('.remove-term', termEl).addEventListener('click', () => {
            removeTerm(tIdx);
        });
...
function populateExamOptions() {
    ...
    addBtn.addEventListener('click', (ev) => {
        ...
        if (!program) {
            alert('Pick AP, IB, CLEP, or Other');
            return;
        }
        if (!exam) {
        ...
        if (program !== 'Other' && !Number.isFinite(score)) {
...
function savePlanToStorage() {
    const payload = {
        terms: state.terms,
        rights: {
            auto: state.rights.auto,
            allow: Array.from(state.rights.allow)
        },
    ...
    for (const t of terms) {
        for (const it of t.items) {
            ...
            rows.push({
                ...
                units,
                title: c?.Title ?? '',
                isPass: PASSING_GE.has(it.grade)
            });
        }
    }
...
function renderRightsControls() {
    ...
    $('#rights-auto').onchange = () => {
        state.rights.auto = $('#rights-auto').checked;
        recalcAll();
    };
...
function renderRightsReadout(allowSet) {
    ...
    div.innerHTML = '';
    if (!allowSet.size) {
        div.textContent = 'No eligible catalog years yet.';
        return;
    }
...
function evaluateGE(ge, transcript, includeExams = true) {
    ...
    function pickForArea(areaObj, used) {
        ...
        function countAreasContaining(code) {
            let cnt = 0;
            for (const s of Object.values(bucketMap))
                if (s.has(code)) cnt++;
            return cnt;
        }
        ...
        if (area === '6A' && chosen.some(c => c.startsWith('EXAM:Other:IGETC 6A'))) {
            return {
                chosen,
                units,
                satisfied: chosen.length >= needCourses
            };
        }
    ...
    function satisfyRequires(requireObj, used, alloc) {
        ...
        for (const a of fromAreas) {
            for (const c of (areaCandidates[a] || [])) {
                if (!used.has(c) && labSet.has(c)) pool.push({
                    a,
                    c,
                    u: unitsFor(c)
                });
    ...
    function walk(node) {
        ...
        if (node.op === 'AND') {
            ...
            if (groupMinCourses || groupMinUnits) {
                ...
                out.push({
                    ok: haveCourses >= groupMinCourses && haveUnits >= groupMinUnits
                });
            }
        ...
        if (node.op === 'OR') {
            ...
            function restore(snap) {
                used.clear();
                for (const v of snap.used) used.add(v);
    ...
    for (const [area, set] of Object.entries(bucketMap)) {
        const a = {
            have: alloc[area]?.length || 0,
            units: sum((alloc[area] || []).map(c => unitsFor(c)))
        };
...
function geCompletionForProgram(prog, tr) {
    ...
    if (certPattern) {
        ...
        if (!candidates.length) {
            return {
                pct: 0,
                label: `${certPattern.toUpperCase()} not available under rights`
            };
        }
    ...
    if (!needsLocal && !needsADTGE) return {
        pct: 1,
        label: 'No GE required (Certificate)'
    };
    ...
    if (!candidates.length) return {
        pct: 0,
        label: 'No eligible GE pattern under rights'
    };
            ...
            const rows = sec.items.map(it => {
                ...
                if (!it.met && r.prereqDetails) {
                    const prereqInfos = it.options
                        ...
                        .map(opt => ({
                            code: opt,
                            info: r.prereqDetails.get(opt)
                        }));

                    if (prereqInfos.length > 0) {
                        const prereqLines = prereqInfos.map(({
                            code,
                            info
                        }) => {
...
function recalcAll() {
    ...
    if (totalUnits >= 60) {
        $('#units-progress-bar').style.background = 'linear-gradient(90deg, var(--success), #2ecc71)';
    } else {
        $('#units-progress-bar').style.background = 'linear-gradient(90deg, var(--warning), var(--accent))';
    }
...
$('#prog-dept-filter').addEventListener('input', () => {
    renderPrograms();
});
