...
# Keys to drop from course JSON anywhere they appear
UNWANTED_KEYS = {
    "FullTitleDisplay",
    "TermsAndSections",
    "LocationsDisplay",
...
CODE_RE = re.compile(r"([A-Za-z]+)-(\d+)")  # AB-15 -> AB 15
  ...
  if isinstance(item, dict):
    ...
    for k, v in item.items():
      ...
      if k == "CourseTitleDisplay":
        # Normalize into CourseCode
        new_item["CourseCode"] = transform_course_title(v)
      else:
        new_item[k] = clean_item(v)

    # Merge Requisites (RequirementCode -> CompletionOrder) into CourseRequisites
    ...
    # Drop source arrays after consolidation
